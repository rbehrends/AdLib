...
Str *ReadFile(FILE *fp) {
  char buffer[65536];
  require(fp != NULL, "file handle is NULL");
  StrList *result = new StrList();
  for (;;) {
...
Str *BuildCommand(Str *prog, StrList *args) {
...
Opt<FileInfo *> FileStat(const char *path, bool follow_links) {
  FileInfo info;
  if (FileStat(info, path, follow_links))
    return Some(new FileInfo(info));
  else
    return None<FileInfo *>();
}

Opt<FileInfo *> FileStat(Str *path, bool follow_links) {
...
static void WalkDir(StrList *acc, const char *path, int mode) {
  StrList *files = ListFiles(path);
  if (!files)
    return;
  FileInfo info;
...
StrList *ListFileTree(const char *path, int mode) {
  StrList *result = new StrList();
  FileInfo info;
...
StrList *ListFileTree(Str *path, int mode) {
...
Str *NormalizePath(Str *path) {
  ...
  StrList *parts = path->split(PathSeparator);
  StrList *result = new StrList();
...
Str *ProgramPath() {
  ...
  if (arg0) {
    ...
    if (!arg0->starts_with(PathSeparator)) {
      ...
      StrList *paths = path ? path->split(':') : new StrList();
