...
void Main() {
  StrList *arr = new StrList();
  Str *collect = new Str();

  arr->add(S("x"))->add(S("y"))->add(S("abc"));
  arr = arr->clone();
  for (StrList::Each iter(arr); iter; ++iter) {
  ...
  StrSet *mapped = dset->map(capitalize);
  ...
  StrList *files = L("alpha.c alpha.h beta.c beta.h");
  StrList *headers = files->filter(F(ends_with, S(".h")));
